...
public sealed class TodoList : AggregateRoot<TodoListId>
{
    ...
    private TodoList(TodoListId id, TodoListTitle title, Colour colour) : base(id)
    {
        Title = title;
        Colour = colour;
    ...
    public static Result<TodoList> Create(string title, string? colourCode = null)
    {
        ...
        var list = new TodoList(TodoListId.New(), titleResult.Value, colourResult.Value);
