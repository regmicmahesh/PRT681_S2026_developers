...
internal sealed class CreateTodoListCommandHandler(ITodoListRepository repository, IUnitOfWork unitOfWork)
    : IRequestHandler<CreateTodoListCommand, Result<Guid>>
{
    ...
    {
        var result = TodoList.Create(request.Title, request.Colour);
        if (result.IsFailure)
