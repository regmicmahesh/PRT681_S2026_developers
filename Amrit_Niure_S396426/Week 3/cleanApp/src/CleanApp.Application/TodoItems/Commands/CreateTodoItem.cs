...
internal sealed class CreateTodoItemCommandHandler(
    ...
    ICurrentUserService currentUser) : IRequestHandler<CreateTodoItemCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateTodoItemCommand request, CancellationToken cancellationToken)
    {
        ...
        var itemResult = TodoItem.Create(currentUser.UserId, listId, request.Title, request.Priority, request.ReminderUtc, request.Note);
